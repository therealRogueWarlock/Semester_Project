...
public class ColourITProjectManagement
{
	...
	public ColourITProjectManagement()
	{
		...
		for (int i = 0; i < projectList.getProjects().size(); i++)
		{
			...
			for (String name : names)
			{
				...
				requirement.getTaskList().addTask(new Task(("randomTask" + j), names[j], 10 + j));

				Task task = requirement.getTaskList().getTaskByName(("randomTask" + j));
				task.getDocumentations().add(new Documentation(names[j], 10 + j, MyDate.now()));
	...
	{
		projectList.getProjectByName(projectName).getTeamMemberList().removeTeamMember(memberName);
	}
	...
	public void addRequirement(String projectName, String requirementName, int timeEstimate, int priority)
	{
		...
		projectList.getProjectByName(projectName).getRequirementList().addRequirement(requirement);
	}
